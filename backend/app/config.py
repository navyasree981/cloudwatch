import os

# PostgreSQL
POSTGRES_URI = os.getenv("POSTGRES_URI", "postgresql://postgres:Jungkook1!@localhost:5432/cloudwatch")

# MongoDB
API_KEY = "fb23af25eda4f16a60eb16a48f7ca7e8"
MONGODB_URI = os.getenv("MONGODB_URI", "mongodb+srv://navyasree:<db_password>@cloudwatch.tom4vt5.mongodb.net/cloudwatch?retryWrites=true&w=majority&appName=cloudwatch")
MONGODB_DB = os.getenv("MONGODB_DB", "cloudwatch")